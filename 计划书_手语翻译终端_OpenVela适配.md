...
>
> **进度快照（2026-08-15 晚）**：Phase 0（环境准备）✅ 100%；Phase 1（BSP 移植）**~95%——固件 `nuttx.bin` 编译成功（591KB），编译层面里程碑 M1 达成**；剩余为真机烧录验证（板子到手后：esptool 烧录 → 串口看 `nsh>` 启动）。硬件资料已从官方 PDF 确认（屏幕 EK73217BCGA、音频 ES8311、C6 模组等，详见 `硬件规格速查表_ESP32P4.md`；**注意文档为 P4X 新硬件，板子到手需确认丝印**）。详见 `开发进度日志.md` 与 `移植调研报告_ESP32P4.md`。
